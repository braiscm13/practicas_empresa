...
/**
 * Manager invoked to register variables for groups of columns. </br> A
 * ColumnsGroupVariable is read and transformed into a JRDesignVariable. </br>
 * </br>
 * 
 * @see DJGroup
 */
public class ColumnsGroupVariablesRegistrationManager extends AbstractEntityRegistrationManager {
	...
	protected void registerEntity(Entity entity) {
		...
		try {
			String name = columnsGroupVariable.getColumnToApplyOperation().getGroupVariableName(this.type, this.columnToGroupByProperty);
			// String name = this.getDjd().getName() +"_" +
			// columnsGroupVariable.getColumnToApplyOperation().getGroupVariableName(type,
			...
			if (columnsGroupVariable.getValueExpression() == null) {
				...
				ColumnsGroupVariablesRegistrationManager.log.debug("registering group variable " + jrVariable.getName() + " ("
	...
	/**
	 * Registers the parameter for the value formatter for the given variable
	 * and puts it's implementation in the parameters map.
	 * 
	 * @param djVariable
	 * @param variableName
	 */
	protected void registerValueFormatter(DJGroupVariable djVariable, String variableName) {
		...
		ColumnsGroupVariablesRegistrationManager.log.debug("Registering value formatter parameter for property " + dparam.getName());
		try {
			this.getDjd().addParameter(dparam);
		} catch (JRException e) {
			throw new EntitiesRegistrationException(e.getMessage(), e);
		}
	...
	protected Object transformEntity(Entity entity) {
		...
		ColumnsGroupVariablesRegistrationManager.log.debug("transforming group variable " + variableName);
